...
async def get_acquisition_focus_scores(acquisition_id: str):
    ...
    if scores:
        try:
            ...
            logger.warning(f"Could not calculate statistics for acquisition {acquisition_id}: {e}")
        except Exception as e:
            logger.error(f"Unexpected error calculating statistics for {acquisition_id}: {e}")
    ...
    logger.info(f"Returning {len(tiles_data)} focus scores for acquisition {acquisition_id}")
    return response
