...
async def test_list_cutting_sessions_filtered(
    ...
    response_block = await async_client.get(
        f"/api/v2/cutting-sessions?block_id={test_block.block_id}"
    )
    ...
    response_specimen = await async_client.get(
        f"/api/v2/cutting-sessions?specimen_id={test_specimen.specimen_id}"
    )
...
@pytest.mark.asyncio
async def test_create_cutting_session(
...
@pytest.mark.asyncio
async def test_get_cutting_session_not_found(
    async_client: AsyncClient, test_specimen, test_block
):
...
async def test_delete_cutting_session(async_client: AsyncClient, test_block):
    ...
    delete_response = await async_client.delete(
        f"/api/v2/cutting-sessions/{session_id_hr}"
    )
...
@pytest.mark.asyncio
async def test_delete_cutting_session_with_sections(
...
async def test_get_cutting_session_sections(
    ...
    assert (
        response_data[0]["cutting_session_id"]
        == test_cutting_session.cutting_session_id
    )
