...
from temdb.models.v2.substrate import (
    Substrate,
    SubstrateCreate,
    SubstrateUpdate,
)
...
async def list_substrates(
    ...
    status: Optional[str] = Query(
        None, description="Filter by substrate status (e.g., 'new', 'used')"
    ),
    ...
    substrates = (
        await Substrate.find(query_filter, fetch_links=False)
        .skip(skip)
        .limit(limit)
        .to_list()
    )
...
@substrate_api.post(
    "/substrates", response_model=Substrate, status_code=status.HTTP_201_CREATED
)
async def create_substrate(substrate_data: SubstrateCreate):
    """
    Create a new substrate.

    Checks if a substrate with the same `media_id` already exists.
    """
    ...
    # Fetch the newly created substrate to return it (ensures all defaults/DB fields are present)
    # Note: Substrate model doesn't have links to fetch currently.
...
async def update_substrate(media_id: str, updated_fields: SubstrateUpdate = Body(...)):
    ...
    # If any field was updated, set the updated_at timestamp and save
    if needs_save:
...
async def delete_substrate(media_id: str):
    ...
    Checks if any Sections are linked to this substrate before deletion.
    """
    substrate = await Substrate.find_one(Substrate.media_id == media_id)
    if not substrate:
        raise HTTPException(
