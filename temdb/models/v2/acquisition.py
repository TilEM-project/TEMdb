...
class StorageLocation(BaseModel):
    location_type: str = Field(
        ..., description="Type of storage location, e.g. local, s3, etc."
    )
    base_path: str = Field(..., description="Base path of storage location")
    is_current: bool = Field(
        ..., description="Whether this is the current storage location"
    )
...
class LensCorrectionModel(BaseModel):
    ...
    type: str = Field(
        ...,
        description="'Transform type as defined in Render Transform Spec. ie 'leaf' 'interpolated' 'list' 'ref'",
    )
...
class Calibration(BaseModel):
    ...
    lens_model: Optional[LensCorrectionModel] = Field(
        None, description="Lens correction model"
    )
    aperture_centroid: Optional[List[float]] = Field(
        None, description="Aperture centroid in stage coordinates in nm"
    )
...
class AcquisitionCreate(BaseModel):
    ...
    acquisition_settings: AcquisitionParams = Field(
        ..., description="Acquisition settings of acquisition"
    )
    calibration_info: Optional[Calibration] = Field(
        None, description="Calibration information of acquisition"
    )
    status: AcquisitionStatus = Field(
        default=AcquisitionStatus.IMAGING, description="Status of acquisition"
    )
    tilt_angle: float = Field(..., description="Tilt angle of acquisition in degrees")
    lens_correction: bool = Field(
        ..., description="Whether this acquisition is a lens correction calibration"
    )
    ...
    sub_region: Optional[Dict[str, int]] = Field(
        None, description="Sub region of acquisition"
    )
    replaces_acquisition_id: Optional[int] = Field(
        None, description="ID of acquisition this acquisition replaces"
    )


class AcquisitionUpdate(BaseModel):
    hardware_settings: Optional[HardwareParams] = Field(
        None, description="Hardware settings of acquisition"
    )
    acquisition_settings: Optional[AcquisitionParams] = Field(
        None, description="Acquisition settings of acquisition"
    )
    calibration_info: Optional[Dict] = Field(
        None, description="Calibration information of acquisition"
    )
    status: Optional[AcquisitionStatus] = Field(
        None, description="Status of acquisition"
    )
    tilt_angle: Optional[float] = Field(
        None, description="Tilt angle of acquisition in degrees"
    )
    lens_correction: Optional[bool] = Field(
        None, description="Whether this acquisition is a lens correction calibration"
    )
    end_time: Optional[datetime] = Field(None, description="End time of acquisition")
    montage_set_name: Optional[str] = Field(None, description="Name of montage set")
    sub_region: Optional[Dict[str, int]] = Field(
    ...
    storage_locations: Optional[List[StorageLocation]] = Field(
        None, description="Storage locations of acquisition"
    )
...
class Acquisition(Document):
    ...
    acquisition_task_ref: Link[AcquisitionTask] = Field(
    ...
    class Settings:
        ...
        indexes = [
            ...
            IndexModel(
                [("montage_set_name", ASCENDING)], name="montage_set_index", sparse=True
            ),
            IndexModel(
    ...
    async def create_acquisition(cls, **kwargs):
        ...
        latest_acquisition = (
            await cls.find(Acquisition.roi_id == roi.id).sort(-cls.version).first()
        )
