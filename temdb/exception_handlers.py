...
class ResourceInUseError(BaseError):
    """Example: Raised when trying to delete a resource that's still linked."""

    def __init__(
        self, resource_type: str, resource_id: str, context: Optional[dict] = None
    ):
        detail = f"{resource_type} '{resource_id}' cannot be deleted because it is still in use or linked by other resources."
        super().__init__(
            detail,
...
async def duplicate_key_exception_handler(request: Request, exc: DuplicateKeyError):
    ...
    error_content = APIErrorResponse(
        detail=f"Resource creation failed due to a conflict. A resource with the same unique identifier ('{conflicting_key}') already exists.",
...
async def document_not_found_exception_handler(request: Request, exc: DocumentNotFound):
    ...
    error_content = APIErrorResponse(
        detail="The requested resource was not found.", error_code="RESOURCE_NOT_FOUND"
    )
...
async def business_logic_exception_handler(request: Request, exc: BaseError):
    ...
    error_content = APIErrorResponse(
        detail=exc.detail, error_code=exc.error_code, context=exc.context
    )
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    logger.exception(
        f"Unhandled exception during request to {request.url}", exc_info=exc
    )
