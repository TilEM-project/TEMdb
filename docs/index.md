...
7.  **[Dataset](models/Dataset.md):** A named collection that groups the imaging work (ROIs, tasks, acquisitions, and tiles) for a specimen. Datasets can nest via `parent_dataset_id`, and their `tile_hash_modulus` drives how tiles are physically partitioned in the database.
8.  **[Acquisition Task](models/AcquisitionTask.md):** A pure imaging plan for a specific ROI within a dataset. Tasks carry no status — execution state is derived from their acquisition runs, and re-planning supersedes a task via `superseded_by`.
9.  **[Acquisition](models/Acquisition.md):** The actual imaging run for an ROI, capturing hardware settings, calibration info, and resulting collection of image data. Carries three orthogonal status axes (terminal `status`, `qc_state`, `transfer_state`) and references the [Microscope](models/Microscope.md) that executed it and, for montages, a [Lens Correction](models/LensCorrection.md).
10.  **[Tile](models/Tile.md):** Individual image frames (tiles) are captured during an acquisition, forming a montage. Each tile has associated metadata like position and quality score metrics and file path. Tiles are keyed by `(dataset_id, run_id, raster_index)` and partitioned per [Dataset](models/Dataset.md).
...
* **Schema Diagrams:** Visualize the relationships between data models:
    * [Full Schema Diagram](./schema_erd.md)
