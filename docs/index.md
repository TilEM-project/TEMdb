...
7.  **[Acquisition Task](models/AcquisitionTask.md):** A pure imaging plan for a specific ROI. Tasks carry no status — execution state is derived from their acquisition runs, and re-planning supersedes a task via `superseded_by`.
8.  **[Acquisition](models/Acquisition.md):** The actual imaging run for an ROI, capturing hardware settings, calibration info, and resulting collection of image data. Carries three orthogonal status axes (terminal `status`, `qc_state`, `transfer_state`) and references the [Microscope](models/Microscope.md) that executed it and, for montages, a [Lens Correction](models/LensCorrection.md).
9.  **[Tile](models/Tile.md):** Individual image frames (tiles) are captured during an acquisition, forming a montage. Each tile has associated metadata like position and quality score metrics and file path. Tiles are keyed by `(dataset_id, run_id, raster_index)` and partitioned per [Dataset](models/Dataset.md).
